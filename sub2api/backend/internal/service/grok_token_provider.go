...
func grokOAuthRequestAccountEligibilityError(account *Account) error {
	// Token fetch must work even when Schedulable=false (admin-disabled / dead-marked).
	// Only refuse if the account itself is gone/inactive or not Grok OAuth.
	if account == nil || !account.IsGrokOAuth() || !account.IsActive() {
		return errOAuthRefreshAccountStateChanged
	}
