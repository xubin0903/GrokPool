...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if s.service.concurrencyService != nil {
		// Grok free pools: never sticky-wait while other accounts may have free slots.
		forceGrokEscape := account != nil && account.IsGrok()
		if (escapeCfg.enabled || forceGrokEscape) && acquireErr == nil && result != nil && !result.Acquired {
			errorRate, ttft, _ := s.stats.snapshot(accountID)
			reason := "concurrency_full"
			if forceGrokEscape && !escapeCfg.enabled {
				reason = "grok_pool_escape"
			}
			slog.Info("sticky_escape_triggered",
				"account_id", accountID,
				"reason", reason,
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	// Grok free pools: always probe beyond TopK so idle accounts with remaining
	// quota are acquired immediately instead of queueing on a busy TopK account.
	...
	for i := range candidates {
		...
		quotaHeadroomFactor := 0.0
			...
			// Grok free accounts: if operator left quota_headroom at 0, apply a mild
			// default so remaining tokens still influence selection without starving load spread.
			if quotaHeadroomWeight == 0 && item.account != nil && item.account.IsGrok() {
				quotaHeadroomWeight = 1.0
			}
			if quotaHeadroomWeight > 0 {
				if item.account != nil && item.account.IsGrok() {
					// Mild remaining preference with diminishing returns so traffic spreads
					// across mid-full free accounts instead of piling onto one fullest.
					quotaHeadroomFactor = math.Sqrt(grokTokenHeadroomFactor(item.account))
				} else {
		...
		item.score = weights.Priority*priorityFactor +
			...
			quotaHeadroomWeight*quotaHeadroomFactor +
...
func (s *defaultOpenAIAccountScheduler) finishLoadBalanceSelectionFallback(
	...
	// Last-chance free-slot acquire across the full selection order (incl. overflow).
	// Prevents queueing on a busy TopK account while idle remaining-quota accounts exist.
	if s.service != nil && s.service.concurrencyService != nil {
		if result, compactBlockedAcquire, acquireErr := s.tryAcquireOpenAISelectionOrderWithBudget(ctx, req, attempt.selectionOrder, budget); acquireErr != nil {
			return nil, candidateCount, topK, loadSkew, acquireErr
		} else if result != nil && result.Acquired {
			return result, candidateCount, topK, loadSkew, nil
		} else if compactBlockedAcquire {
			attempt.compactBlocked = true
		}
	}
	...
	for pass := 0; pass < passes; pass++ {
		...
		for _, candidate := range attempt.selectionOrder {
			...
			if candidate.loadKnown {
				cur = candidate.loadInfo.CurrentConcurrency
			}
			if waitCandidate == nil || cur < waitLoad {
				waitCandidate = fresh
				waitLoad = cur
			}
			if normalizeOpenAICompatiblePlatform(req.Platform) != PlatformGrok {
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatible(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) bool {
	...
	// Grok free-tier OAuth (~1M token windows) must be filtered here as well.
	// Otherwise exhausted free accounts can monopolize TopK and force retries.
	if paused, _ := shouldAutoPauseGrokAccountByQuota(account); paused {
		return false
	}
	// Request-fit: avoid assigning non-sticky work to near-empty free accounts.
	// Sticky sessions keep the same account until hard exhaustion to protect context.
	if account.IsGrok() {
		sticky := req.StickyAccountID > 0 && account.ID == req.StickyAccountID
		if !grokAccountFitsEstimatedTokens(account, 0, sticky) {
			return false
		}
	}
...
func (s *OpenAIGatewayService) openAIWSSchedulerWeights() GatewayOpenAIWSSchedulerScoreWeightsView {
	...
	return GatewayOpenAIWSSchedulerScoreWeightsView{
		...
		TTFT:          0.5,
		Reset:         0.0,
		// Mild free-tier remaining preference; load/queue dominate to avoid pile-up.
		QuotaHeadroom: 1.0,
		UpstreamCost:  0.0,
		Previous:      5.0,
		SessionSticky: 3.0,
...
func buildOpenAIAccountSchedulerScoreSnapshot(
	...
	for _, candidate := range candidates {
		...
		if weights.QuotaHeadroom > 0 {
			if candidate.account != nil && candidate.account.IsGrok() {
				quotaHeadroomFactor = grokTokenHeadroomFactor(candidate.account)
			} else {
