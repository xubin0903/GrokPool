...
func shouldAutoPauseGrokQuotaWindow(name string, window *xai.QuotaWindow, now time.Time) (bool, openAIQuotaAutoPauseDecision) {
	...
	// Hard pause only when truly exhausted. Soft headroom / request-fit is handled by
	// GrokPool scoring (prefer high remaining) so tiny residual free-tier quotas are
	// not wasted on sticky mid-conversation traffic.
...
// grokTokenRemainingLimit returns (remaining, limit, ok) from the passive xAI quota snapshot.
// Free Grok OAuth accounts typically expose a ~1M token window via rate-limit headers.
func grokTokenRemainingLimit(account *Account) (remaining int64, limit int64, ok bool) {
	if account == nil || !account.IsGrok() {
		return 0, 0, false
	}
	snapshot, err := grokQuotaSnapshotFromExtra(account.Extra)
	if err != nil || snapshot == nil || snapshot.Tokens == nil {
		return 0, 0, false
	}
	if snapshot.Tokens.Limit == nil || snapshot.Tokens.Remaining == nil || *snapshot.Tokens.Limit <= 0 {
		return 0, 0, false
	}
	now := time.Now()
	if snapshot.Tokens.ResetUnix != nil && *snapshot.Tokens.ResetUnix > 0 && !now.Before(time.Unix(*snapshot.Tokens.ResetUnix, 0)) {
		// Window already reset; treat as full until a fresh observation arrives.
		return *snapshot.Tokens.Limit, *snapshot.Tokens.Limit, true
	}
	if grokQuotaSnapshotStaleForPause(snapshot, now) {
		return 0, 0, false
	}
	rem := *snapshot.Tokens.Remaining
	if rem < 0 {
		rem = 0
	}
	return rem, *snapshot.Tokens.Limit, true
}

// grokTokenHeadroomFactor scores free/paid Grok accounts by remaining token ratio.
// Higher remaining → higher score. Unknown snapshot stays neutral so cold accounts
// remain selectable until the first upstream response observes headers.
func grokTokenHeadroomFactor(account *Account) float64 {
	remaining, limit, ok := grokTokenRemainingLimit(account)
	if !ok || limit <= 0 {
		return openAIQuotaHeadroomNeutralFactor
	}
	return clamp01(float64(remaining) / float64(limit))
}

// grokAccountFitsEstimatedTokens implements request-fit gating for free-tier pools.
// Free accounts only have ~1M tokens/day — do NOT burn residual quota with a large
// soft floor. Sticky sessions always continue until hard exhaustion.
func grokAccountFitsEstimatedTokens(account *Account, estimatedTokens int64, sticky bool) bool {
	remaining, limit, ok := grokTokenRemainingLimit(account)
	if !ok {
		return true
	}
	...
	// Free pool: any positive remaining is usable. A 32k floor was wasting real
	// daily quota across many near-empty accounts while other traffic queued.
	if isKnownGrokFreeAccount(account) {
		return true
	}
	// Paid / unknown: keep a small floor so near-empty accounts are not preferred
	// for brand-new non-sticky traffic, but do not hide large residual windows.
	softFloor := int64(4_096)
	if limit > 0 {
		// Also allow accounts whose remaining is still meaningful relative to limit.
		pctFloor := limit / 200 // 0.5%
		if pctFloor > softFloor {
			softFloor = pctFloor
		}
		if softFloor > 16_384 {
			softFloor = 16_384
	...
	if estimatedTokens <= 0 {
		return remaining >= softFloor || remaining > softFloor/4
	}
	need := estimatedTokens + estimatedTokens/5 + 2_048
	if need < softFloor {
		need = softFloor
	}
