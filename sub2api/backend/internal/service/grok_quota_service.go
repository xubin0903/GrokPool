...
//
// Important: xAI billing endpoints often return 403 for freshly imported Free
// OAuth accounts even when the account is fully usable. Do NOT short-circuit on
// billing alone when status is 403 — always follow with an active model probe so
// the UI does not stick on "forbidden" until a manual refresh.
func (s *GrokQuotaService) QueryQuota(ctx context.Context, accountID int64) (*GrokQuotaProbeResult, error) {
	...
	billingForbidden := billingStatusForbidden(billingResult)
	if billingErr == nil && billingResult != nil && grokBillingHasAuthoritativeQuota(billingResult.Billing) && !billingForbidden {
		return billingResult, nil
	}

	probeResult, probeErr := s.ProbeUsage(ctx, accountID)
	if probeErr != nil {
		// Prefer returning a hybrid result with ProbeError so dead-account
		// classifiers can see permission-denied chat 403 bodies.
		if probeResult != nil {
			if billingResult != nil {
				probeResult.Source = "hybrid_probe"
				if probeResult.Billing == nil {
			...
			if probeResult.ProbeError == "" {
				probeResult.ProbeError = probeErr.Error()
			}
...
func billingStatusForbidden(result *GrokQuotaProbeResult) bool {
	if result == nil {
		return false
	}
	...
	if result.Billing != nil && (result.Billing.StatusCode == http.StatusForbidden ||
		result.Billing.WeeklyStatusCode == http.StatusForbidden ||
		result.Billing.MonthlyStatusCode == http.StatusForbidden) {
		return true
	}
	return false
}

// GrokAccountLiveness is the result of classifying whether a Grok OAuth account
// is still usable for chat traffic (not just media/billing).
type GrokAccountLiveness struct {
	...
	HeadersObserved  bool   `json:"headers_observed"`
	ProbeError       string `json:"probe_error,omitempty"`
	EntitlementHint  string `json:"entitlement_hint,omitempty"`
}

// ClassifyGrokAccountLiveness decides dead vs alive from a quota probe.
//
// Dead = permanently unusable by upstream for chat (revoked / missing creds /
// entitlement denied / hard 401). Billing-only 403 on Free accounts is NOT dead.
func ClassifyGrokAccountLiveness(accountID int64, result *GrokQuotaProbeResult, probeErr error) GrokAccountLiveness {
	out := GrokAccountLiveness{AccountID: accountID, Unknown: true, Reason: "unprobed"}
	if probeErr != nil {
		...
		if status := infraerrors.FromError(probeErr); status != nil {
			...
			if status.Reason != "" {
				msg = strings.ToLower(status.Reason + " " + status.Message)
			}
		}
		out.StatusCode = code
		out.ProbeError = probeErr.Error()
		if isGrokPermanentDeadMessage(msg) || code == http.StatusUnauthorized {
			out.Dead = true
			out.Unknown = false
			out.Alive = false
			out.Reason = "permanent_credential_or_auth_failure"
			return out
		}
		out.Reason = "probe_error_transient_or_unknown"
		return out
	}
	if result == nil {
		out.Reason = "empty_probe_result"
		return out
	}

	out.Source = result.Source
	out.StatusCode = result.StatusCode
	out.HeadersObserved = result.HeadersObserved
	out.ProbeError = result.ProbeError
	if result.Snapshot != nil {
		if result.Snapshot.EntitlementStatus != "" {
			out.EntitlementHint = result.Snapshot.EntitlementStatus
		}
		if result.StatusCode == 0 {
			out.StatusCode = result.Snapshot.StatusCode
		}
		if result.Snapshot.HeadersObserved {
			out.HeadersObserved = true
		}
	}

	// Successful active/hybrid observation = usable.
	if out.HeadersObserved && out.StatusCode >= 200 && out.StatusCode < 300 {
		out.Alive = true
		out.Dead = false
		out.Unknown = false
		out.Reason = "active_probe_ok"
		return out
	}
	// Billing-authoritative paid path without errors.
	if result.Billing != nil && grokBillingHasAuthoritativeQuota(result.Billing) &&
		result.Billing.StatusCode >= 200 && result.Billing.StatusCode < 300 &&
		result.ProbeError == "" {
		out.Alive = true
		out.Dead = false
		out.Unknown = false
		out.Reason = "billing_ok"
		return out
	}
	...
	if out.StatusCode == http.StatusUnauthorized || isGrokPermanentDeadMessage(combined) {
		...
		out.Alive = false
		out.Unknown = false
		out.Reason = "permanent_upstream_denial"
		return out
	}

	// Active probe hard 403 (not billing-only flaky 403) after hybrid attempt.
	// Chat permission-denied bodies are permanent dead for free-pool ops.
	if (result.Source == "active_probe" || result.Source == "hybrid_probe") &&
		out.StatusCode == http.StatusForbidden {
		if isGrokPermanentDeadMessage(combined) ||
			strings.Contains(combined, "permission-denied") ||
			strings.Contains(combined, "chat endpoint") {
			out.Dead = true
			out.Alive = false
			out.Unknown = false
			out.Reason = "chat_permission_denied"
			return out
		}
	}

	// Billing-only 403 without active headers: Free-account flaky state, not dead.
	if billingStatusForbidden(result) && result.Source == "billing_probe" && !out.HeadersObserved && result.ProbeError == "" {
		out.Alive = false
		out.Dead = false
		out.Unknown = true
		out.Reason = "billing_forbidden_needs_active_probe"
		return out
	}
...
func isGrokPermanentDeadMessage(msg string) bool {
...
func (s *GrokQuotaService) ProbeAccountLiveness(ctx context.Context, accountID int64) GrokAccountLiveness {
	if s == nil {
		return GrokAccountLiveness{AccountID: accountID, Unknown: true, Reason: "quota_service_nil"}
	}
	result, err := s.QueryQuota(ctx, accountID)
	return ClassifyGrokAccountLiveness(accountID, result, err)
}
...
func (s *GrokQuotaService) probeUsage(ctx context.Context, accountID int64) (*GrokQuotaProbeResult, error) {
	...
	if resp.StatusCode >= 400 {
		...
		// Persist a failed active-probe snapshot so the UI/classifier can see 403
		// even when billing earlier returned a flaky forbidden.
		result.ProbeError = bodyText
		if bodyText == "" {
		...
		// Return both result and error so callers that only check err still work,
		// while hybrid QueryQuota can attach ProbeError onto billing result.
