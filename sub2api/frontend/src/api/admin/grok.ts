...
export interface GrokProbeDeadItem {
  account_id: number
  name?: string
  email?: string
  alive: boolean
...
export interface GrokProbeDeadResponse {
  total: number
  alive: number
  dead: number
  unknown: number
  deleted: number
  items: GrokProbeDeadItem[]
}

export async function probeDeadAccounts(
  accountIds: number[],
  deleteDead = false
): Promise<GrokProbeDeadResponse> {
  const { data } = await apiClient.post<GrokProbeDeadResponse>(
    '/admin/grok/accounts/probe-dead',
    { account_ids: accountIds, delete_dead: deleteDead },
    // bulk hybrid probes can take a while
    { timeout: Math.min(300_000, 20_000 + accountIds.length * 15_000) }
  )
  return data
}
