export default {
    accounts: {
      ...
      bulkActions: {
        ...
        batchTest: '批量测试',
        batchTestConfirm: '将对选中的 {count} 个账号执行连接测试（发送 hi）。继续？',
        batchTestNeedSelection: '请先勾选要测试的账号',
        batchTestSuccess: '批量测试完成：全部成功 {count} 个',
        batchTestPartial: '批量测试完成：成功 {success}，失败 {failed}（失败ID示例: {failedIds}）',
        probeDead: '探测死号',
        deleteDead: '删除死号',
        deadToolsHint: 'Grok 死号工具：先勾选账号，再探测。真死号会自动停调度并标红；假 forbidden 不会误杀。',
        probeDeadConfirm: '将对选中的 {count} 个账号执行 Grok 活性探测。仅把上游永久封禁/凭证失效判为死号（不会把 billing 403 误判当死号）。继续？',
        deleteDeadConfirm: '将对选中的 {count} 个账号探测，并直接删除判定为死号的账号。此操作不可撤销。继续？',
        probeDeadSuccess: '探测完成：活 {alive} / 死 {dead} / 未知 {unknown}',
        deleteDeadSuccess: '已删除 {deleted} 个死号（探测：活 {alive} / 死 {dead} / 未知 {unknown}）',
        deleteDeadNone: '没有可删除的死号（探测：活 {alive} / 死 {dead} / 未知 {unknown}）。未知号不会被删除，避免误杀。',
        probeDeadNoGrok: '请先勾选要探测的 Grok 账号（可点“本页全选”）',
        probeDeadLimit: '批量探测数量已放宽（不再限制 50）',
        probeDeadNeedSelection: '请先勾选账号，再点探测死号',
        deadMarked: '死号',
