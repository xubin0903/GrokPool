<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-gray-200 bg-white/70 px-3 py-2 text-xs dark:border-gray-700 dark:bg-gray-900/40">
          <span class="font-semibold text-gray-500 dark:text-gray-400">{{ t('admin.accounts.quickLinks.title') }}</span>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/dashboard">{{ t('admin.accounts.quickLinks.userDashboard') }}</RouterLink>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/keys">{{ t('admin.accounts.quickLinks.userKeys') }}</RouterLink>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/admin/groups">{{ t('admin.accounts.quickLinks.groups') }}</RouterLink>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/admin/users">{{ t('admin.accounts.quickLinks.users') }}</RouterLink>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/admin/proxies">{{ t('admin.accounts.quickLinks.proxies') }}</RouterLink>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/admin/usage">{{ t('admin.accounts.quickLinks.usage') }}</RouterLink>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/admin/ops">{{ t('admin.accounts.quickLinks.ops') }}</RouterLink>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/admin/redeem">{{ t('admin.accounts.quickLinks.redeem') }}</RouterLink>
          <RouterLink class="rounded-full bg-gray-100 px-2.5 py-1 text-gray-700 hover:bg-primary-50 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-900/30 dark:hover:text-primary-300" to="/admin/settings">{{ t('admin.accounts.quickLinks.settings') }}</RouterLink>
        </div>
        ...
        >
          ...
          <template #cell-name="{ row, value }">
            <div class="flex flex-col">
              ...
              <span
                v-if="isDeadMarked(row)"
                class="mt-0.5 inline-flex w-fit items-center rounded bg-rose-100 px-1.5 py-0.5 text-[10px] font-semibold text-rose-700 dark:bg-rose-900/40 dark:text-rose-300"
                :title="row.error_message || row.notes || ''"
              >
                {{ t('admin.accounts.bulkActions.deadMarked') }}
              </span>
              <span
...
function isDeadMarked(row: any): boolean {
  const err = String(row?.error_message || '')
  const notes = String(row?.notes || '')
  return err.startsWith('DEAD:') || notes.startsWith('DEAD:')
}
...
const selectedGrokOAuthIds = () => {
  const idSet = new Set(selIds.value)
  return accounts.value
    .filter((a) => idSet.has(a.id) && a.platform === 'grok' && a.type === 'oauth')
    .map((a) => a.id)
}

const runBulkGrokDeadProbe = async (deleteDead: boolean) => {
  // If nothing selected, auto-select current page Grok OAuth accounts.
  if (selIds.value.length === 0) {
    const pageGrok = accounts.value
      .filter((a) => a.platform === 'grok' && a.type === 'oauth')
      .map((a) => a.id)
    if (pageGrok.length === 0) {
      appStore.showError(t('admin.accounts.bulkActions.probeDeadNoGrok'))
      return
    }
    // select them for visual feedback
    pageGrok.forEach((id) => {
      if (!selIds.value.includes(id)) {
        // use selection helper if available via toggleSel
        try { toggleSel(id) } catch { /* ignore */ }
      }
    })
  }

  let accountIDs = selectedGrokOAuthIds()
  if (accountIDs.length === 0) {
  ...
  const confirmKey = deleteDead
    ? 'admin.accounts.bulkActions.deleteDeadConfirm'
    : 'admin.accounts.bulkActions.probeDeadConfirm'
  if (!confirm(t(confirmKey, { count: accountIDs.length }))) return
  try {
    const result = await adminAPI.grok.probeDeadAccounts(accountIDs, deleteDead)
    // Immediately reflect dead/alive on the current table without waiting full reload.
    const byId = new Map((result.items || []).map((it) => [it.account_id, it]))
    if (deleteDead) {
      const deletedIds = new Set(
        (result.items || []).filter((it) => it.deleted).map((it) => it.account_id)
      )
      if (deletedIds.size > 0) {
        accounts.value = accounts.value.filter((acc) => !deletedIds.has(acc.id))
      }
    }
    accounts.value = accounts.value.map((acc) => {
      const it = byId.get(acc.id)
      if (!it || it.deleted) return acc
      if (it.dead) {
        return {
          ...acc,
          schedulable: false,
          notes: `DEAD: ${it.reason || ''}${it.probe_error ? ' | ' + it.probe_error : ''}`,
          // keep status active-looking; error_message only if already present from old bug
          error_message: String(acc.error_message || '').startsWith('DEAD:') ? '' : acc.error_message
        }
      }
      if (it.alive) {
        // Clear stale DEAD badge from previous false positives.
        const err = String(acc.error_message || '')
        const notes = String(acc.notes || '')
        if (err.startsWith('DEAD:') || notes.startsWith('DEAD:') || acc.schedulable === false) {
          return {
            ...acc,
            schedulable: true,
            error_message: err.startsWith('DEAD:') ? '' : acc.error_message,
            notes: notes.startsWith('DEAD:') ? '' : acc.notes
          }
        }
      }
      return acc
    ...
    if (deleteDead) {
      if ((result.deleted || 0) === 0 && (result.dead || 0) === 0) {
        appStore.showError(
          t('admin.accounts.bulkActions.deleteDeadNone', toastPayload)
        )
      } else {
        appStore.showSuccess(t('admin.accounts.bulkActions.deleteDeadSuccess', toastPayload))
      }
    } else {
      appStore.showSuccess(t('admin.accounts.bulkActions.probeDeadSuccess', toastPayload))
    }
    if ((result.deleted || 0) > 0) clearSelection()
    reload()
  } catch (error) {
    console.error('Failed to probe/delete dead Grok accounts:', error)
